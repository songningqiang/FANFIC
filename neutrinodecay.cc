...
//by integrating neutrino sources at different redshifts and by assuming certain source flavor compositions
...
int main()
{
	...
	std::vector<double> initialflavor {1./3., 2./3., 0.}; //initial flavor, must sum up to 1	
	...
	for (int i = 0; i < Max_samp; ++i)
	{
		...
		//specify the prior range of m/\tau
	    double kappa = oscprior.flatPrior(oscprior.rand01(), 0., 1.e-4);
