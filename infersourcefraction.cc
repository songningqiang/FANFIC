...
// in the source (k_\pi, k_\mu, k_n) (pion decay, damped muon, neutrond decay)
...
int main()
{
	...
	std::cout << "Output will be written in the file " << foutput << ", in the format of f_piondecay, f_dampedmuon, f_neutrondecay, chi2." << std::endl; 
	...
	FILE *fp;
 	...
 	fprintf(fp, "#k_piondecay k_dampedmuon k_neutrondecay chi2\n");
	...
	for (int i = 0; i < Max_samp; ++i)
	{
		...
		std::vector<double> f_i = oscprior.randomInitialFlavor();
	    for (int k = 0; k < 3; ++k)
	    {
	    	comp_i[k] += f_i[0]*flav_pion[k] + f_i[1]*flav_muon[k] + f_i[2]*flav_neutron[k]; 
	    }
